...
class Dinos {
  constructor(atrib) {
    ...
    this.diet = atrib.diet;
    this.weight = atrib.weight;
    this.length = atrib.length;
    this.period = atrib.period;
  }
  roar() {
...
const Steg = new Dinos({
  name: "stegosaurus",
  diet: "herbivorous",
  weight: "2000kg",
  length: "9m",
  period: "Late Jurassics"
});
// velociraptor, carnivorous, 15kg, 1.8m, Late Cretaceous
const Velo = new Dinos({
  name: "velociraptor",
  diet: "herbivorous",
  weight: "15kg",
  length: "8m",
...
console.log(Tyno.period);

// Create a new roar method for the tyrannosaurus.  When called, return "RAWERSRARARWERSARARARRRR!" Log the result.
console.log(Tyno.roar());
...
const unisWithUni = [];
graduates.filter(function(ele) {
  if (ele.university.includes("Uni")) {
    unisWithUni.push(ele.university);
  }
});
